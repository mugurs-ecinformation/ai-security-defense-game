...
// ========== Güvenli Leaderboard sabitleri (DOLDUR) ==========
const WEBHOOK_URL   = 'BURAYA_WEB_APP_EXEC_URL'; // Apps Script Web App URL (/exec)
const API_KEY       = 'BURAYA_GIZLI_API_KEY';    // Apps Script ile aynı
const SHARED_SECRET = 'BURAYA_SHARED_SECRET';    // Apps Script ile aynı
...
// ========== Chat / Terminal ==========
function appendChat(msg){ const cb=$('#chat-box'); const div=document.createElement('div'); div.className='chat-msg'; const h=document.createElement('span'); h.style.color='var(--accent)'; h.style.fontSize='10px'; h.textContent='[INCOMING]'; const br=document.createElement('br'); const body=document.createElement('span'); body.textContent=msg; div.appendChild(h); div.appendChild(br); div.appendChild(body); cb.appendChild(div); cb.scrollTop=cb.scrollHeight; }
...
function renderMaskMission(m){
  ...
  const wrap=document.createElement('div'); wrap.className='mask-wrap'; wrap.id='mask-editor';
  ...
  parts.forEach(p=>{
    if(p.startsWith('{{')&&p.endsWith('}}')){
      ...
      wrap.appendChild(sp); wrap.appendChild(document.createTextNode(' '));
    }else{ wrap.appendChild(document.createTextNode(p)); }
  });
  area.appendChild(wrap);
  const ctrl=document.createElement('div'); ctrl.className='mask-controls';
  ...
  const reset=document.createElement('button'); reset.className='choice-btn'; reset.style.borderColor='var(--accent)'; reset.style.color='var(--accent)'; reset.textContent=t('retry'); reset.onclick=()=>renderMaskMission(m);
  const note=document.createElement('div'); note.className='note'; note.textContent=t('tip_mask');
  ctrl.appendChild(send); ctrl.appendChild(reset); area.appendChild(ctrl); area.appendChild(note);
}
...
function evaluateMaskMission(m){
  const unmasked=Array.from(document.querySelectorAll('#mask-editor .pii')).filter(x=>!x.classList.contains('masked'));
...
function renderScannerMission(m){
  ...
  const timer=document.createElement('div'); timer.id='sc-timer'; timer.style.color='var(--neon-red)'; timer.textContent=formatTime(m.duration);
  ...
  let tsec=m.duration, hits=0, misses=0, falses=0;
  function tick(){ tsec--; timer.textContent=formatTime(tsec); if(tsec<=0) finish(); }
  scTick=setInterval(tick,1000);

  function spawn(){
    const p=document.createElement('div'); const isDanger=Math.random()>0.6;
    p.className='packet '+(isDanger?'danger':'safe');
    p.textContent=isDanger?t('risk_pkt'):t('safe_pkt');
    p.style.left=(Math.random()*80+5)+'%';
    p.onclick=()=>{ if(isDanger){ hits++; score+=m.hitReward; Sound.play('snd-success'); } else { falses++; hp-=m.falsePenalty; if(hp<0) hp=0; Sound.play('snd-alarm'); } p.remove(); updateStats(); };
    stream.appendChild(p);
    setTimeout(()=>{ if(p.parentNode){ if(isDanger){ misses++; hp-=m.missPenalty; if(hp<0) hp=0; updateStats(); } p.remove(); } },5200);
  }
  function updateStats(){ $('#hp-val').textContent=hp+'%'; $('#score-val').textContent=String(score); refreshRiskUI(); }
  ...
  function finish(){
    ...
    const ok = hits >= m.successThreshold && misses < (m.successThreshold+2);
    if(!ok){
      Sound.play('snd-alarm');
      $('#fact-text').textContent=(m.fact_fail[LANG]||m.fact_fail['nl'])+
        `\n\n${LANG==='nl'?'Geïntercepteerd':'Intercepted'}: ${hits}\n${LANG==='nl'?'Gemist':'Missed'}: ${misses}\n${LANG==='nl'?'Vals alarm':'False alarm'}: ${falses}`;
      decisions.push({missionIndex:step, choiceText:`Scanner: hits=${hits}, miss=${misses}`, ok:false, feedback:'Netwerkbewaking onvoldoende'});
      $('#log-line').textContent=t('detected');
    } else {
      ...
      $('#fact-text').textContent=(m.fact_ok[LANG]||m.fact_ok['nl'])+
        `\n\n${LANG==='nl'?'Geïntercepteerd':'Intercepted'}: ${hits}\n${LANG==='nl'?'Gemist':'Missed'}: ${misses}\n${LANG==='nl'?'Vals alarm':'False alarm'}: ${falses}`;
      decisions.push({missionIndex:step, choiceText:`Scanner: hits=${hits}, miss=${misses}`, ok:true, feedback:'Netwerkbewaking effectief'});
      $('#log-line').textContent=t('accepted');
    }
    pushBadge(); showFact();
  }
...
function renderMission(){
  ...
  if(step>=missions.length){ showEndScreen(); return; }
  const m=missions[step];
  appendChat(m.msg[LANG]||m.msg['nl']); typeTerminal(m.terminal[LANG]||m.terminal['nl']);
  const area=$('#choices-area'); area.innerHTML='';
  ...
  $('#log-line').textContent = `${t('mission')} ${step+1}/${missions.length} ${corporateMode ? t('corporate') : t('standard')} …`;
}
function handleChoice(m,c){
  $all('#choices-area .choice-btn').forEach(btn=>btn.disabled=true);
  ...
  const fbText=(c.ok?'[OK] ':'[ALERT] ')+(c.feedback[LANG]||c.feedback['nl']); typeTerminal(fbText+'\n\n> '+t('see_case'));
  $('#fact-text').textContent=(m.fact && (m.fact[LANG]||m.fact['nl']))||''; showFact();
}
...
function showEndScreen(){
  ...
  decisions.forEach(d=>{ const m=missions[d.missionIndex]; const box=document.createElement('div'); box.className='summary'; const st=d.ok?`<span class="ok">${t('correct')}</span>`:`<span class="fail">${t('wrong')}</span>`; box.innerHTML=`<div><strong>${t('mission')} ${d.missionIndex+1}:</strong> ${m.sender}</div><div style="margin:6px 0; color:#bbb;">${m.msg[LANG]||m.msg['nl']}</div><div>${t('choice')}: <em>${d.choiceText}</em> — ${st}</div><div style="margin-top:6px;">Feedback: ${d.feedback}</div>`; list.appendChild(box); });
  typeTerminal(LANG==='nl'?'> OPERATIE VOLTOOID. RESULTATEN WORDEN GETOOND...':'> OPERATION COMPLETE. SHOWING RESULTS...');
  // 🔴 Güvenli skor gönderimi (async; await gerekmez)
  submitScore();
...
function replay(){
  ...
  $('#terminal-content').textContent=''; $('#choices-area').innerHTML=''; $('#end-modal').style.display='none'; renderMission();
}

// ========== Güvenli gönderim yardımcıları ==========
function base64Url(buf){ let bin=''; const bytes=new Uint8Array(buf); for(let i=0;i<bytes.length;i++) bin+=String.fromCharCode(bytes[i]); return btoa(bin).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,''); }
function hex(buf){ const b=new Uint8Array(buf); return Array.from(b).map(x=>x.toString(16).padStart(2,'0')).join(''); }
function canonicalStringify(obj){ if(obj===null||typeof obj!=='object') return JSON.stringify(obj); if(Array.isArray(obj)) return '['+obj.map(canonicalStringify).join(',')+']'; const keys=Object.keys(obj).sort(); const parts=keys.map(k=>JSON.stringify(k)+':'+canonicalStringify(obj[k])); return '{'+parts.join(',')+'}'; }
async function sha256Hex(text){ const enc=new TextEncoder(); const dig=await crypto.subtle.digest('SHA-256', enc.encode(text)); return hex(dig); }
async function hmacSignB64Url(secret, message){ const enc=new TextEncoder(); const key=await crypto.subtle.importKey('raw', enc.encode(secret), {name:'HMAC', hash:'SHA-256'}, false, ['sign']); const sig=await crypto.subtle.sign('HMAC', key, enc.encode(message)); return base64Url(sig); }

// ========== Güvenli submit ==========
async function submitScore(){
  try{
    if(!WEBHOOK_URL || WEBHOOK_URL.indexOf('/exec')===-1) return;
    const missions=getActiveMissions();
    ...
    const ts=Date.now();
    const sign=await hmacSignB64Url(SHARED_SECRET, `${ts}.${payloadHash}`);

    const body={ apiKey: API_KEY, ts, hash: payloadHash, sign, data };
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  // Oyuncu adı (takma ad)
  if(!PLAYER){
    const promptTxt=t('name_prompt');
    const val=(typeof window.prompt==='function') ? window.prompt(promptTxt) : '';
    PLAYER=(val && val.trim()) ? val.trim() : 'Anonymous';
